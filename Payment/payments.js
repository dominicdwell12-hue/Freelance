...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await pool.query(
        `UPDATE payments SET provider = $1, provider_ref = $2 WHERE id = $3`,
...
router.post('/webhook/:provider', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE payments SET status = 'held', held_at = now()
    ...
    if (result.rows.length > 0) {
      ...
      console.log(`Escrow held for payment ${result.rows[0].id}`);
    }
  } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO disputes (job_id, payment_id, raised_by, reason) VALUES ($1, $2, $3, $4)`,
        [payment.job_id, payment.id, req.user.id, req.body.reason]
      );
      await client.query(`UPDATE jobs SET status = 'disputed', updated_at = now() WHERE id = $1`, [payment.job_id]);
